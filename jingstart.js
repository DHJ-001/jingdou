...
//3.不断查找某控件父元素，并点击(解决模拟器和手机不查找元素不一致问题)
function up_click(x) {
...
function check_intolist(){
  while(1){
    sleep(2000);
    if(textMatches('去完成|已完成').findOne(3000)) {toastLog('回到任务列表中');break}
        else {toastLog('进入任务列表失败\n 尝试返回');back();sleep(1000);}}}
		

//点击xx项目的'去完成'按钮
function just_doit(matchtext, failshow){
    if(textContains(matchtext).findOne(2000).parent().children()[0]) position_click(textContains(matchtext).findOne(2000).parent().children()[0]);
...
function cai_ni_xihuan(){
  while(1){sleep(2000);
    ...
    if (!textStartsWith('猜你喜欢的商品').exists()) return false;
