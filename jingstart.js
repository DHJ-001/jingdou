...
function check_where(){while(1){
  if(textMatches('去完成|已完成').findOne(2000)) return toastLog('chua~进列表')
  		else if(text('领京豆').findOne(1000)) {toastLog('发现处于京东首页');return getinto_task();}
  			else if(packageName('com.jingdong.app.mall').exists()) {back();sleep(1000);return toastLog('尝试返回');}
  				else if(packageName('com.tencent.mm').exists()) {back();sleep(1000);back();return toastLog('检测到跳转微信 尝试返回');}
...
while(1){
  others_task();
  if(zhongjie_caini && textContains('猜你喜欢的商品').boundsInside(0, 0, dev_width, dev_hight / 1.5).findOne(2000)) cai_ni_xihuan();
}