...
function check_where(){while(1){
  if(textMatches('去完成|已完成').findOne(3000)) return toastLog('chua~进列表')
  	else if(packageName('com.jingdong.app.mall').exists()) {toastLog('尝试返回');back();sleep(1000);}
  		else {toastLog('检测到已经离开京东！ 正在返回');getinto_task();}}}
...
function click_QWC(){
    ...
    if(qwc) {position_click(qwc);sleep(2000); back();check_where();}}
